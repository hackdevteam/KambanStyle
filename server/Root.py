...
class Root():
    def __init__(self):
        self.__config = ""
        self.__persistence = FilesystemPersistence(Url(Url.SCHEME_FILE, "localhost", "filesystem_database"))
        self.config_server_to_serve_static_content()
        self.mount_end_point(API_BOARD)
        self.mount_end_point(API_COLUMN)
        self.mount_end_point(API_TASK)
        self.start_server()
    ...
    def mount_end_point(self, end_point_uri):
        cherrypy.tree.mount(BoardApiEndPoint(PersistenceManager(self.__persistence)),
                            end_point_uri,
