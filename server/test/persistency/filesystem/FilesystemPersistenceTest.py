...
class BoardManagerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.peristency = FilesystemPersistence(Url(Url.SCHEME_FILE, "localhost", BASE_FOLDER))
    ...
    def test_getting_all_the_user_boards_ids(self):
        ...
        self.assertEqual(2, len(self.peristency.get_boards_ids()))

    def test_save_boards_with_the_same_name(self):
        self.peristency.save_board(Board("Board 1"))
        self.peristency.save_board(Board("Board 1"))
        self.peristency.save_board(Board("Board 1"))
        self.assertEqual(3, len(self.peristency.get_boards_ids()))

    def test_load_board(self):
        board = Board("Mock Board")
        serialize_object(os.path.join(BASE_FOLDER), board.get_id() + ".brd", board)
        loaded_board = self.peristency.load_board(self.peristency.get_boards_ids()[0])
    ...
    def test_save_board(self):
        ...
        self.peristency.save_board(board_1)
        self.peristency.save_board(board_2)
        self.assertEqual(2, len(self.peristency.get_boards_ids()))
        self.assertTrue(board_2.get_id() in self.peristency.get_boards_ids())
        self.assertTrue(board_1.get_id() in self.peristency.get_boards_ids())
    ...
    def test_load_column(self):
        ...
        loaded_column = self.peristency.load_column(self.peristency.get_columns_ids(TEST_BOARD_ID)[0], TEST_BOARD_ID)
    ...
    def test_save_column(self):
        ...
        self.peristency.save_column(column_1)
        self.peristency.save_column(column_2)
        self.peristency.save_column(column_3)
    ...
    def test_get_columns_ids(self):
        self.peristency.save_column(Column("column 1", TEST_BOARD_ID))
        self.peristency.save_column(Column("column 2", TEST_BOARD_ID))
        self.peristency.save_column(Column("column 3", TEST_BOARD_ID))
        self.assertEqual(3, len(self.peristency.get_columns_ids(TEST_BOARD_ID)))

    def test_save_a_column_with_duplicated_name(self):
        self.peristency.save_column(Column("column 1", TEST_BOARD_ID))
        self.peristency.save_column(Column("column 1", TEST_BOARD_ID))
        self.assertEqual(2, len(self.peristency.get_columns_ids(TEST_BOARD_ID)))
    ...
    def test_get_tasks_ids(self):
        ...
        self.peristency.save_task(Task("task 1", "", column.get_id()))
        self.peristency.save_task(Task("task 1", "", column.get_id()))
        self.peristency.save_task(Task("task 1", "", column.get_id()))
        self.assertEqual(3, len(self.peristency.get_tasks_ids(column.get_id())))

    def test_save_a_task(self):
        column = Column("column 1", TEST_BOARD_ID)
        task = Task("title task 1", "description", column.get_id())
        self.peristency.save_column(column)
        self.peristency.save_task(task)
        self.assertEqual(task.get_id(), self.peristency.get_tasks_ids(column.get_id())[0])

    def test_delete_a_task(self):
        column = Column("column 1", TEST_BOARD_ID)
        os.makedirs(os.path.join(BASE_FOLDER, TEST_BOARD_ID, str(column.get_id())))
        task_1 = Task("task 1", "", column.get_id())
        self.peristency.save_task(task_1)
        self.peristency.save_task(Task("task 2", "", column.get_id()))
        self.peristency.save_task(Task("task 3", "", column.get_id()))
        self.assertEqual(3, len(self.peristency.get_tasks_ids(column.get_id())))
        self.peristency.delete_task(task_1.get_id())
        self.assertEqual(2, len(self.peristency.get_tasks_ids(column.get_id())))
    ...
    def test_delete_an_non_existent_task(self):
        ...
        self.peristency.delete_task(task_1.get_id())
        self.peristency.delete_task(task_1.get_id())
        self.assertEqual(1, len(self.peristency.get_tasks_ids(column.get_id())))