...
class BoardManagerTest(unittest.TestCase):
    ...
    def test_save_boards_with_the_same_name(self):
        self.persistence.save_board(Board("Board 1"))
        self.persistence.save_board(Board("Board 1"))
        self.persistence.save_board(Board("Board 1"))
        self.assertEqual(3, len(self.persistence.get_boards_ids()))

    def test_load_board(self):
        board = Board("Mock Board")
        serialize_object(os.path.join(BASE_FOLDER), board.get_id() + ".brd", board)
        loaded_board = self.persistence.load_board(self.persistence.get_boards_ids()[0])
    ...
    def test_save_board(self):
        ...
        self.persistence.save_board(board_1)
        self.persistence.save_board(board_2)
        self.assertEqual(2, len(self.persistence.get_boards_ids()))
        self.assertTrue(board_2.get_id() in self.persistence.get_boards_ids())
        self.assertTrue(board_1.get_id() in self.persistence.get_boards_ids())
    ...
    def test_load_column(self):
        ...
        loaded_column = self.persistence.load_column(self.persistence.get_columns_ids(TEST_BOARD_ID)[0], TEST_BOARD_ID)
    ...
    def test_save_column(self):
        ...
        self.persistence.save_column(column_1)
        self.persistence.save_column(column_2)
        self.persistence.save_column(column_3)
    ...
    def test_get_columns_ids(self):
        self.persistence.save_column(Column("column 1", TEST_BOARD_ID))
        self.persistence.save_column(Column("column 2", TEST_BOARD_ID))
        self.persistence.save_column(Column("column 3", TEST_BOARD_ID))
        self.assertEqual(3, len(self.persistence.get_columns_ids(TEST_BOARD_ID)))

    def test_save_a_column_with_duplicated_name(self):
        self.persistence.save_column(Column("column 1", TEST_BOARD_ID))
        self.persistence.save_column(Column("column 1", TEST_BOARD_ID))
        self.assertEqual(2, len(self.persistence.get_columns_ids(TEST_BOARD_ID)))
    ...
    def test_get_tasks_ids(self):
        ...
        self.persistence.save_task(Task("task 1", "", column.get_id()))
        self.persistence.save_task(Task("task 1", "", column.get_id()))
        self.persistence.save_task(Task("task 1", "", column.get_id()))
        self.assertEqual(3, len(self.persistence.get_tasks_ids(column.get_id())))

    def test_save_a_task(self):
        column = Column("column 1", TEST_BOARD_ID)
        task = Task("title task 1", "description", column.get_id())
        self.persistence.save_column(column)
        self.persistence.save_task(task)
        self.assertEqual(task.get_id(), self.persistence.get_tasks_ids(column.get_id())[0])
    ...
    def test_delete_a_task(self):
        ...
        self.persistence.save_task(Task("task 2", "", column.get_id()))
        self.persistence.save_task(Task("task 3", "", column.get_id()))
        self.assertEqual(3, len(self.persistence.get_tasks_ids(column.get_id())))
        self.persistence.delete_task(task_1.get_id())
        self.assertEqual(2, len(self.persistence.get_tasks_ids(column.get_id())))
    ...
    def test_delete_an_non_existent_task(self):
        ...
        self.persistence.delete_task(task_1.get_id())
        self.persistence.delete_task(task_1.get_id())
        self.assertEqual(1, len(self.persistence.get_tasks_ids(column.get_id())))